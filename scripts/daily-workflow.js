...
import { consensusAnalysis, qualityCheck } from './report-enhancer.js';
...
操作建议: <推荐的操作和条件>
风险提示: <提示的风险点>
分析框架/独特视角: <博主使用的分析方法论、独特分析框架、有特色的比喻或思考方式，如"四要素共振框架""生死线双框架"。无则写"无">
特色观点/反共识: <与市场主流不同的独特观点、反共识判断，或是他人未提及的特殊逻辑。无则写"无">
...
export function parseArticleFields(text) {
  ...
  for (const line of lines) {
    const m = line.match(/^(来源|板块\/行业|具体标的|市场判断|核心观点|操作建议|风险提示|分析框架\/独特视角|特色观点\/反共识):\s*(.*)/);
    if (m) {
  ...
  return {
    ...
    risk: fields['风险提示'] || '',
    isNonFinance,
    framework: fields['分析框架/独特视角'] || '',
    specialView: fields['特色观点/反共识'] || '',
    isEmpty: ['来源','板块/行业','具体标的','市场判断','核心观点','操作建议','风险提示','分析框架/独特视角','特色观点/反共识'].every(k => !fields[k] || fields[k] === '无'),
  };
}
...
export function buildStructuredReport(articles, targetDate, totalCount) {
  ...
  const frameworks = [];
  for (const a of valid) {
    if (a.framework && a.framework !== '无') {
      frameworks.push({ source: a.source, text: a.framework });
    }
  }
  ...
  const specials = [];
  for (const a of valid) {
    if (a.specialView && a.specialView !== '无') {
      specials.push({ source: a.source, text: a.specialView });
    }
  }
...
export async function analyzeArticles(downloaded) {
  ...
  // Phase 2c: LLM 模式分析（共识/分歧/特色）
  const hasContent = a => !a.isEmpty && !a.isNonFinance;
  const valid = articlesParsed.filter(hasContent);
  console.log('\n🧠  LLM 模式分析（共识/分歧/特色观点）');
  const patternSection = await consensusAnalysis(valid);
  let finalContent = structuredReport;
  if (patternSection) {
    const separator = '\n---\n\n### 原文清单';
  ...
  const reportSizeKB = (finalContent.length / 1024).toFixed(1);
